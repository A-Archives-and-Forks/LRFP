...
##### After the BOCHK application upgraded its security measures on December 21, 2025 (version 7.4.9), it pops up the [security warning page](https://www.bochk.com/dam/securitywarning.html?reason=0&manufacturer=OnePlus&model=PJZ110&android=36&version=8.2.1&root=40AFAADB&return=0&srcApp=bochk) or raises biometric authentication failures

- Make sure that BOCHK does not pop up the [security warning page](https://www.bochk.com/dam/securitywarning.html?reason=0&manufacturer=OnePlus&model=PJZ110&android=36&version=8.2.1&root=40AFAADB&return=0&srcApp=bochk) on your device when there are completely no rooting or injection environments
- Please install the build artifact generated by the latest successful workflow run in the GitHub Actions of [TEESimulator](https://github.com/JingMatrix/TEESimulator/actions) that was triggered by a push event and whose triggering actor is not a bot account
- On Android 12 or above, if the issue persists, please clear the key blob cache, or even clear the entire application data
- If the above solutions still fail to resolve the issue, please refer to the following table, where OxygenOS was installed on the OnePlus 9P device, while the other systems were installed on the OnePlus 13 device, BOCHK being available when online is normal (it will directly show "no information" when offline) in the fingerprint login column, and "Crash" refers to BOCHK closing itself and popping up the [security warning page](https://www.bochk.com/dam/securitywarning.html?reason=0&manufacturer=OnePlus&model=PJZ110&android=36&version=8.2.1&root=40AFAADB&return=0&srcApp=bochk)

| System | Tricky Store or its variants | ``target.txt`` or the "apps" field in ``config.json`` | After lauching BOCHK | Fingerprint login |
| - | - | - | - | - |
| OxygenOS 11 / PixelOS 16 | Tricky Store / TEESimulator 3.x | Not including BOCHK | No obvious abnormalities observed | Available when online |
| OxygenOS 11 / PixelOS 16 | Tricky Store / TEESimulator 3.x | Including BOCHK (any suffix) | No obvious abnormalities observed | Unavailable |
| OxygenOS 11 / PixelOS 16 | TEESimulator-RS 280 | Any configuration | No obvious abnormalities observed | Available when online |
| OxygenOS 11 | TEESimulator-RS 307 | Any configuration | Unable to boot | —— |
| OxygenOS 11 | TEESimulator 4.x | Not including BOCHK | No obvious abnormalities observed | Available when online |
| OxygenOS 11 | TEESimulator 4.x | Including BOCHK (any suffix) | No obvious abnormalities observed | Unavailable |
| PixelOS 16 | TEESimulator-RS 307 | Any configuration | No obvious abnormalities observed | Available when online |
| Evolution X 16 / uwuAOSP 16 | Tricky Store / TEESimulator | Any configuration | Crashes about 3 seconds after launch | —— |
| Evolution X 16 / uwuAOSP 16 | TEESimulator-RS | Not including BOCHK or using ! suffix | Crashes about 3 seconds after launch | —— |
| Evolution X 16 / uwuAOSP 16 | TEESimulator-RS | ``com.bochk.app.aos`` or ``com.bochk.app.aos?`` | With ``remote_provisioning.hostname`` set to ``remoteprovisioning.googleapis.com`` in the system.prop of any rooting-layer system module, after reboot, BOCHK crashes about 23 seconds after launch when online / about 3 seconds after launch when offline | Available when online |
| PixelOS 17 | Tricky Store / TEESimluator 3.x / TEESimulator-RS | Any configuration | No obvious abnormalities observed | Unavailable |
| PixelOS 17 | TEESimulator 4.x | Including BOCHK | No obvious abnormalities observed | Available when online |
...
- 安装最新版 [Play Integrity fix](https://github.com/KOWX712/PlayIntegrityFix/actions) 模块
- 不建议使用插件进行过检因为对检测应用注入虽然能够过检掉调试模式（可疑级别）但会暴露 Xposed/Edxposed/LSPosed 注入/钩子（确信级别）反而得不偿失
...
##### 中银香港应用程序在 2025 年 12 月 21 日（7.4.9 版本）升级了保安措施后弹出[安全警告网页](https://www.bochk.com/dam/securitywarning.html?reason=0&manufacturer=OnePlus&model=PJZ110&android=36&version=8.2.1&root=40AFAADB&return=0&srcApp=bochk)或无法进行生物认证

- 请确保在完全没有 root 或注入环境的情况下，BOCHK 不会在你的设备上弹出[安全警告网页](https://www.bochk.com/dam/securitywarning.html?reason=0&manufacturer=OnePlus&model=PJZ110&android=36&version=8.2.1&root=40AFAADB&return=0&srcApp=bochk)
- 请安装 [TEESimulator](https://github.com/JingMatrix/TEESimulator/actions) 的 GitHub Actions 中的由 push 事件触发且触发者不为机器人的最新且成功的工作流所生成的构建
- 在 Android 12 或更高版本上，如果问题仍然存在，请清除 key blob 缓存，或甚至清除整个应用数据
- 若上述方案仍然无法解决问题，请参阅下表，其中 OxygenOS 在一加 9P 上，其余系统在一加 13 上，指纹登录中 BOCHK 联网状态下可用是正常现象（离线状态下会直接显示没有资讯），闪退指 BOCHK 自我关闭并弹出[安全警告网页](https://www.bochk.com/dam/securitywarning.html?reason=0&manufacturer=OnePlus&model=PJZ110&android=36&version=8.2.1&root=40AFAADB&return=0&srcApp=bochk)

| 系统 | Tricky Store 或其变体 | target.txt 或 config.json 中的 apps | 启动 BOCHK 现象 | 指纹登录 |
| - | - | - | - | - |
| OxygenOS 11 / PixelOS 16 | Tricky Store / TEESimulator 3.x | 不包含 BOCHK | 未见有明显异常 | 联网状态下可用 |
| OxygenOS 11 / PixelOS 16 | Tricky Store / TEESimulator 3.x | 包含 BOCHK（任意后缀） | 未见有明显异常 | 不可用 |
| OxygenOS 11 / PixelOS 16 | TEESimulator-RS 280 | 任意配置 | 未见有明显异常 | 联网状态下可用 |
| OxygenOS 11 | TEESimulator-RS 307 | 任意配置 | 无法开机 | —— |
| OxygenOS 11 | TEESimulator 4.x | 不包含 BOCHK | 未见有明显异常 | 联网状态下可用 |
| OxygenOS 11 | TEESimulator 4.x | 包含 BOCHK（任意后缀） | 未见有明显异常 | 不可用 |
| PixelOS 16 | TEESimulator-RS 307 | 任意配置 | 未见有明显异常 | 联网状态下可用 |
| Evolution X 16 / uwuAOSP 16 | Tricky Store / TEESimulator | 任意配置 | 打开后约 3 秒闪退 | —— |
| Evolution X 16 / uwuAOSP 16 | TEESimulator-RS | 不包含 BOCHK 或使用 ! 后缀 | 打开后约 3 秒闪退 | —— |
| Evolution X 16 / uwuAOSP 16 | TEESimulator-RS | ``com.bochk.app.aos`` 或 ``com.bochk.app.aos?`` | 任意 root 层系统模块的 system.prop 中设置 ``remote_provisioning.hostname`` 为 ``remoteprovisioning.googleapis.com`` 重启后联网状态下启动 BOCHK 后约 23 秒闪退 / 离线状态下启动 BOCHK 后约 3 秒闪退 | 联网状态下可用 |
| PixelOS 17 | Tricky Store / TEESimluator 3.x / TEESimulator-RS | 任意配置 | 未见有明显异常 | 不可用 |
| PixelOS 17 | TEESimulator 4.x | 包含 BOCHK | 未见有明显异常 | 联网状态下可用 |
